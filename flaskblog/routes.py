...
def register():
    ...
    if form.validate_on_submit():
        ...
        user=User(username=form.username.data, name=form.name.data, email=form.email.data, password=hased_password)
        db.session.add(user)
        db.session.commit()
...
def new_post():
    form = PostForm()
    if form.validate_on_submit():
        post = Post( title=form.title.data,type=form.type.data, branch=form.branch.data,role=form.role.data, round= form.round.data, interview_date=form.interview_date.data , content=form.content.data, verdict=form.verdict.data, author=current_user)
        db.session.add(post)
        db.session.commit()
